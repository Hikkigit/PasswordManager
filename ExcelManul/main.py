...
import time
from ExcelManul.SendEmail  import sendemail
...
for i in range(worktable.nrows):
    ...
    #for j in range(worktable.ncols):
        coltype=worktable.cell_type(i,8)
        if coltype==3: #data类型为3
            #print("Xldate=",worktable.cell(i,8))
            Time=xlrd.xldate.xldate_as_tuple(worktable.cell_value(i,8),workbook.datemode)
            ...
            print(formatday)
            disdays=(datetime.now()-formatday).days#date格式相减
            #dismonth=(datetime.now()-formatday).months
            print("disdays=",disdays)
            #print("dismonth=", dismonth)
            if disdays>=10:
                ID=str(worktable.cell_value(i,4))
                id=ID.split('.')
                msg="请修改"+id[0]+"的密码！"
                print(msg)
